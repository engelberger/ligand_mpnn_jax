...
def main(args) -> None:
    ...
    if args.bias_AA:
        ...
        for i, AA in enumerate(a1):
            bias_AA = jax.ops.index_update(
                bias_AA, jax.ops.index[restype_str_to_int[AA]], a2[i]
            )
    ...
    omit_AA = jnp.array([AA in omit_AA_list for AA in alphabet], dtype=jnp.float32)
    ...
    for pdb in pdb_paths:
        ...
        R_idx_list = list(protein_dict["R_idx"])  # residue indices
                ...
                if residue_name in encoded_residues:
                    ...
                    for amino_acid, v2 in v1.items():
                        if amino_acid in alphabet:
                            ...
                            bias_AA_per_residue = jax.ops.index_update(
                                bias_AA_per_residue, jax.ops.index[i1, j1], v2
                            )
            ...
            for residue_name, v1 in omit_dict.items():
                if residue_name in encoded_residues:
                    ...
                    for amino_acid in v1:
                        if amino_acid in alphabet:
                            j1 = restype_str_to_int[amino_acid]
                            omit_AA_per_residue = jax.ops.index_update(
                                omit_AA_per_residue, jax.ops.index[i1, j1], 1.0
                            )

        fixed_positions = jnp.array(
            [int(item not in fixed_residues) for item in encoded_residues]
        )
        redesigned_positions = jnp.array(
        ...
        if args.verbose:
            if "Y" in list(protein_dict):
                atom_coords = protein_dict["Y"]
                atom_types = list(protein_dict["Y_t"])
                atom_mask = list(protein_dict["Y_m"])
        ...
        for _ in range(args.number_of_batches):
            ...
            output_dict = model.apply(model_params, feature_dict, method=model.sample)
        ...
        S_stack = jnp.concatenate(S_list, axis=0)
        log_probs_stack = jnp.concatenate(log_probs_list, axis=0)
        sampling_probs_stack = jnp.concatenate(sampling_probs_list, axis=0)
        decoding_order_stack = jnp.concatenate(decoding_order_list, axis=0)
        loss_stack = jnp.concatenate(loss_list, axis=0)
        loss_per_residue_stack = jnp.concatenate(loss_per_residue_list, axis=0)
        loss_XY_stack = jnp.concatenate(loss_XY_list, axis=0)

        rec_mask = feature_dict["mask"][:1] * feature_dict["chain_mask"][:1]
        rec_stack = get_seq_rec(feature_dict["S"][:1], S_stack, rec_mask)

        native_seq = "".join([restype_int_to_str[AA] for AA in feature_dict["S"][0]])
        seq_np = np.array(list(native_seq))
        seq_out_str = []
        for mask in protein_dict["mask_c"]:
            seq_out_str += list(seq_np[mask])
        ...
        output_stats_path = base_folder + "stats/" + name + args.file_ending + ".npz"

        out_dict = {}
        out_dict["generated_sequences"] = S_stack
        out_dict["sampling_probs"] = sampling_probs_stack
        out_dict["log_probs"] = log_probs_stack
        out_dict["decoding_order"] = decoding_order_stack
        out_dict["native_sequence"] = feature_dict["S"][0]
        out_dict["mask"] = feature_dict["mask"][0]
        out_dict["chain_mask"] = feature_dict["chain_mask"][0]
        out_dict["seed"] = seed
        out_dict["temperature"] = args.temperature

        if args.save_stats:
            np.savez(output_stats_path, **out_dict)
        ...
        with open(output_fasta, "w") as f:
            ...
            for ix in range(S_stack.shape[0]):
                ...
                seq_rec_print = np.format_float_positional(
                    rec_stack[ix], unique=False, precision=4
                )
                loss_np = np.format_float_positional(
                    np.exp(-loss_stack[ix]), unique=False, precision=4
                )
                loss_XY_np = np.format_float_positional(
                    np.exp(-loss_XY_stack[ix]), unique=False, precision=4
                )
                seq = "".join([restype_int_to_str[AA] for AA in S_stack[ix]])

                # write new sequences into PDB with backbone coordinates
                seq_prody = np.array([restype_1to3[AA] for AA in list(seq)])[
                    None,
                ].repeat(4, axis=1)
                bfactor_prody = loss_per_residue_stack[ix][None, :].repeat(4, axis=1)
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        type=int,
        default=0,
        help="Set seed for jax.random, numpy, and python random.",
    )
    argparser.add_argument(
        "--batch_size",
        type=int,
